...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    // Dành cho Admin: Liệt kê các Đơn hàng theo Trạng thái (Ví dụ: "CHỜ XÁC NHẬN"), Có phân trang
    Page<Order> findByStatus(OrderStatus status, Pageable pageable);

    // Lấy đơn hàng bằng ID NHƯNG bắt buộc Đơn hàng đó phải thuộc Khóa ngoại của chính nó!
    Optional<Order> findByIdAndUserId(Long orderId, Long userId);

    // Lấy đơn hàng của Khách để Thanh toán (Chỉ lấy đơn có trạng thái Chờ thanh toán / Lỗi)
    Optional<Order> findByIdAndUserIdAndStatusIn(Long orderId, Long userId, List<OrderStatus> statuses);

    // Dành cho Cron Job: Tìm các đơn "CHỜ THANH TOÁN" đã quá hạn (VD: 10 phút) để hệ thống tự động Hủy
